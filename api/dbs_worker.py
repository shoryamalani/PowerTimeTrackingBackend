...
def set_up_db_version_7(conn):
    ...
    execute_db.execute_database_command(set_up_connection(),query_sql)[0].commit()
    query_sql = "ALTER TABLE users ALTER COLUMN mobile_devices TYPE json USING mobile_devices;"
    execute_db.execute_database_command(set_up_connection(),query_sql)[0].commit()
    set_db_version(7)

def set_up_db_version_8(conn):
    # change column mobile_devices to json
    query_sql = "ALTER TABLE users ALTER COLUMN mobile_devices TYPE json USING mobile_devices;"
    execute_db.execute_database_command(set_up_connection(),query_sql)[0].commit()
    set_db_version(8)
...
def get_phone_data(phone_id):
    conn = set_up_connection()
    mobile_devices = pypika.Table('mobile_devices')
    query = Query.from_(mobile_devices).select('*').where(mobile_devices.device_id == phone_id)
    data = execute_db.execute_database_command(conn,query.get_sql())[1]
    mobile_device = data.fetchone()
    if mobile_device:
        return mobile_device
